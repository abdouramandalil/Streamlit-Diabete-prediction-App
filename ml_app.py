...
def run_ml_app():
	...
	with col1:
		...
		polydypsia=st.radio("Polydipsia",["No","Yes"])
		...
		genital_trush=st.selectbox("Genital_trush",["No","Yes"])
	...
	with st.expander("Prediction Result"):
		...
		pred_bob=model.predict_proba(single_sample)
		st.write(prediction)
		st.write(pred_bob) 

		if prediction ==1:
			st.warning("Positive Risk {}".format(prediction[0]))
			pred_probability_score={"Negative DM Risk":pred_bob[0][0]*100,"Positive DM Risk":pred_bob[0][1]*100}
